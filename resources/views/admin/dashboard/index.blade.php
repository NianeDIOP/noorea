        ...
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-sm text-gray-600 mt-1">
                    <span class="text-green-600">{{ $stats['active_products'] }} actifs</span>
                </p>
            </div>
    ...
    <div class="stats-card info">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-sm text-gray-600 mt-1">
                    <span class="text-yellow-600">{{ $stats['pending_orders'] }} en attente</span>
                </p>
            </div>
            <div class="stats-card-icon">
...
<div class="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
    ...
    <div class="admin-card">
        <div class="admin-card-header">
            ...
            <a href="{{ route('admin.orders.index') }}" class="btn-admin-secondary text-sm px-4 py-2">
                Voir tout
            </a>
        </div>
        <div class="space-y-4">
    ...
    <div class="admin-card">
        <div class="admin-card-header">
            ...
            <a href="{{ route('admin.products.index') }}" class="btn-admin-secondary text-sm px-4 py-2">
                Voir tout
            </a>
        </div>
        <div class="space-y-4">
            @forelse($top_viewed_products as $product)
                    ...
                    <div class="flex-1 min-w-0">
                        ...
                        <div class="flex items-center space-x-4 mt-1">
                            <span class="text-xs text-purple-600 bg-purple-100 px-2 py-1 rounded">
                                <i class="fas fa-eye mr-1"></i>{{ number_format($product->views) }} vues
                            </span>
                            <span class="text-xs text-green-600 font-medium">{{ $product->formatted_price }}</span>
                        </div>
                    </div>
                </div>
            @empty
...
<div class="admin-card mb-8">
    <div class="admin-card-header">
        ...
        <span class="status-badge danger">{{ $low_stock_products->count() }} produit(s)</span>
    </div>
                ...
                <div class="flex items-center space-x-3">
                    ...
                    <div class="flex-1">
                        ...
                        <p class="text-sm font-semibold text-red-600">
                            Stock: {{ $product->stock_quantity }} unité(s)
                        </p>
                    </div>
                </div>
                <div class="mt-3">
...
async function loadChartData() {
    try {
        // Données des ventes
        const salesResponse = await fetch('{{ route('admin.dashboard.chart-data') }}?type=sales');
        ...
        const categoriesResponse = await fetch('{{ route('admin.dashboard.chart-data') }}?type=categories');
...
async function updateChart(type) {
    try {
        const response = await fetch(`{{ route('admin.dashboard.chart-data') }}?type=sales`);
