...
<div class="flex items-center space-x-3">
    <a href="{{ route('admin.brands.show', $brand) }}" class="btn-admin-secondary">
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
    ...
    <div class="lg:col-span-2">
        <form action="{{ route('admin.brands.update', $brand) }}" method="POST" enctype="multipart/form-data" id="brandForm">
            @csrf
            @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea name="description" 
                                  ...
                                  maxlength="1000">{{ old('description', $brand->description) }}</textarea>
                        @error('description')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
                        <div class="flex justify-between mt-1">
                            ...
                            <p class="text-xs text-gray-500">
                                <span id="description-count">{{ strlen(old('description', $brand->description ?? '')) }}</span>/1000
                            </p>
                        </div>
                    </div>
            ...
            <div class="admin-card mt-6">
                    ...
                    <div class="flex space-x-4">
                        <label class="flex items-center">
                            <input type="radio" name="logo_type" value="upload" class="mr-2" 
                                   {{ !$brand->logo || !filter_var($brand->logo, FILTER_VALIDATE_URL) ? 'checked' : '' }} 
                ...
                <!-- Upload de logo -->
                <div id="upload_section" class="{{ $brand->logo && filter_var($brand->logo, FILTER_VALIDATE_URL) ? 'hidden' : '' }}">
                ...
                <!-- URL de logo -->
                <div id="url_section" class="{{ !$brand->logo || !filter_var($brand->logo, FILTER_VALIDATE_URL) ? 'hidden' : '' }}">
                    ...
                    <div id="url_preview" class="mt-4 {{ filter_var($brand->logo ?? '', FILTER_VALIDATE_URL) ? '' : 'hidden' }}">
                        <img src="{{ filter_var($brand->logo ?? '', FILTER_VALIDATE_URL) ? $brand->logo : '' }}" class="h-32 w-auto rounded-lg shadow-md bg-white border p-2">
                    </div>
                    @error('logo_url')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                ...
                <div class="grid grid-cols-1 gap-6">
                    ...
                    <div>
                        ...
                        <div class="flex justify-between mt-1">
                            ...
                            <p class="text-xs text-gray-500">
                                <span id="meta-description-count">{{ strlen(old('meta_description', $brand->meta_description ?? '')) }}</span>/500
                            </p>
                        </div>
                    </div>
                </div>
            ...
            <div class="space-y-4">
                ...
                <div class="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                    ...
                    <label class="admin-toggle">
                        ...
                        <span class="status-label {{ old('is_active', $brand->is_active ?? true) ? 'active' : 'inactive' }} ml-2" id="statusLabel">
                            {{ old('is_active', $brand->is_active ?? true) ? 'Actif' : 'Inactif' }}
                        </span>
                    </label>
                </div>
                ...
                <div class="space-y-3">
                    ...
                    <a href="{{ route('admin.brands.show', $brand) }}" class="btn-admin-secondary w-full">
        ...
        <div class="admin-card">
            ...
            <div class="space-y-3">
                <div class="flex justify-between items-center">
                    ...
                    <span class="font-semibold text-gray-900">{{ $brand->products()->count() }}</span>
                </div>
                <div class="flex justify-between items-center">
                    <span class="text-sm text-gray-600">Créée le</span>
                    <span class="font-semibold text-gray-900">{{ $brand->created_at->format('d/m/Y') }}</span>
                </div>
                <div class="flex justify-between items-center">
                    <span class="text-sm text-gray-600">Modifiée le</span>
                    <span class="font-semibold text-gray-900">{{ $brand->updated_at->format('d/m/Y') }}</span>
                </div>
            </div>
        </div>
        ...
        <div class="mt-3 text-center">
            ...
            <div class="mt-2 px-7 py-3">
                <p class="text-sm text-gray-500">
                    Êtes-vous sûr de vouloir supprimer la marque "<span class="font-semibold">{{ $brand->name }}</span>" ?
                </p>
