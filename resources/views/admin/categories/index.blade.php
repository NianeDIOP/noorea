...
<div class="flex flex-col sm:flex-row items-start sm:items-center space-y-2 sm:space-y-0 sm:space-x-3">
    <div class="flex items-center space-x-2">
        <div class="flex items-center bg-gray-100 rounded-lg px-3 py-2 text-sm">
            ...
            <span class="font-medium text-gray-700">{{ $categories->total() }} catégorie(s)</span>
        </div>
    ...
    <div class="stats-card primary">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-3xl font-bold text-gray-900 mt-2">{{ $categories->total() }}</p>
            </div>
    ...
    <div class="stats-card success">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-3xl font-bold text-gray-900 mt-2">{{ $categories->where('is_active', true)->count() }}</p>
            </div>
            <div class="stats-card-icon">
    ...
    <div class="stats-card info">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-3xl font-bold text-gray-900 mt-2">{{ $categories->whereNull('parent_id')->count() }}</p>
            </div>
            <div class="stats-card-icon">
    ...
    <div class="stats-card warning">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-3xl font-bold text-gray-900 mt-2">{{ $categories->whereNotNull('parent_id')->count() }}</p>
            </div>
            <div class="stats-card-icon">
    ...
    <div class="admin-card-header">
        ...
        <div class="flex items-center space-x-2">
            <span class="text-sm text-blue-600 bg-blue-50 px-3 py-1 rounded-lg">
                {{ collect(request()->only(['search', 'status', 'parent']))->filter()->count() }} filtre(s) actif(s)
            </span>
            <a href="{{ route('admin.categories.index') }}" class="btn-admin-secondary text-sm">
    ...
    <form method="GET" action="{{ route('admin.categories.index') }}" class="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div class="admin-form-group flex items-end">
            <div class="flex space-x-2 w-full">
                ...
                @if(request()->hasAny(['search', 'status', 'parent']))
                    <a href="{{ route('admin.categories.index') }}" class="btn-admin-secondary px-3" title="Réinitialiser les filtres">
...
<div class="admin-card">
    <div class="admin-card-header">
        ...
        <div class="flex items-center space-x-4">
            <span class="text-sm text-gray-500">
                {{ $categories->firstItem() ?? 0 }}-{{ $categories->lastItem() ?? 0 }} sur {{ $categories->total() }}
            </span>
        ...
        <table class="admin-table">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        <div class="flex flex-col">
                            ...
                            @if($category->description)
                                <span class="text-sm text-gray-500">{{ Str::limit($category->description, 40) }}</span>
                            @endif
                        </div>
                    </td>
                    ...
                    <td>
                        @if($category->parent_id)
                            <div class="flex flex-col">
                                ...
                                <span class="text-xs text-gray-500 mt-1">{{ $category->parent->name }}</span>
                            </div>
                        @else
                            <span class="status-badge primary">Principale</span>
                            @if($category->subcategories->count() > 0)
                                <span class="text-xs text-gray-500 block mt-1">{{ $category->subcategories->count() }} sous-cat.</span>
                            @endif
                        @endif
                    </td>
                    ...
                    <td>
                        <div class="flex items-center space-x-2">
                            <span class="text-lg font-semibold text-gray-900">{{ $category->products_count }}</span>
                            @if($category->products_count > 0)
                                <a href="{{ route('admin.categories.show', $category) }}" class="text-noorea-gold hover:text-noorea-gold/80">
                    ...
                    <!-- Ordre -->
                    <td>
                        <span class="px-2 py-1 bg-gray-100 rounded text-sm font-mono">{{ $category->sort_order }}</span>
                    </td>
                    ...
                    <td class="px-6 py-4">
                        <form action="{{ route('admin.categories.toggle-status', $category) }}" method="POST" class="inline">
                            @csrf
                            <label class="admin-toggle" title="{{ $category->is_active ? 'Désactiver' : 'Activer' }} cette catégorie">
                                <input type="checkbox" {{ $category->is_active ? 'checked' : '' }} 
                                       onchange="confirmToggleStatusInline(this, '{{ $category->is_active ? "désactiver" : "activer" }}', '{{ $category->name }}')">
                                <span class="toggle-slider"></span>
                                <span class="status-label {{ $category->is_active ? 'active' : 'inactive' }}">
                    ...
                    <td>
                        <div class="flex items-center space-x-2">
                            ...
                            @if($category->products_count === 0 && $category->subcategories->count() === 0)
    ...
    <div class="text-center py-12">
        ...
        @if(request()->hasAny(['search', 'status', 'parent']))
            <a href="{{ route('admin.categories.index') }}" class="btn-admin-secondary mr-3">
