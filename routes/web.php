...
// Page d'accueil
// Route de test pour l'upload (à supprimer en production)
...
Route::post('/test-upload', function(Illuminate\Http\Request $request) {
    ...
    try {
        ...
        return redirect()->back()->with('success', 
            "Catégorie créée avec succès! ID: {$category->id}, Image: {$path}, URL: " . Storage::disk('public')->url($path)
        );
...
// Test modification catégorie
Route::get('/test-update/{category}', function(App\Models\Category $category) {
    return view('test.update', compact('category'));
});

Route::post('/test-update/{category}', function(Illuminate\Http\Request $request, App\Models\Category $category) {
    \Log::info('Test Update Category - Request Data:', [
    ...
    try {
        if ($request->hasFile('image')) {
            ...
            return redirect()->back()->with('success', 
                "Image mise à jour avec succès! Nouveau chemin: {$path}, URL: " . Storage::disk('public')->url($path)
            );
        } else {
            return redirect()->back()->withErrors(['image' => 'Aucun fichier reçu']);
        }
